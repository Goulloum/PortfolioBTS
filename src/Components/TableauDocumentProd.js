    ...
    return(
        ...
        <div className="table-document-header">
          <div
            ...
            style={{ opacity: table == "doc" ? "1" : "0.7" }}
            onClick={() => setTable("doc")}
          >
            Documents
          </div>
          <div
            className="title-document"
            onClick={() => setTable("prod")}
            style={{ opacity: table == "prod" ? "1" : "0.7" }}
          >
            Productions
          </div>
        </div>
          ...
          {table == "doc"
            ? props.project.documents.map((document, i) => (
                ...
                >
                  <Link
                    to={
                      "/files/" + props.project.name + "/documents/" + document
                    }
                    target="_blank"
                    download
                    key={i}
                  >
                    <div key={i} className="click">
                      {document}
                      <span className="material-icons pdf-icon">
                        picture_as_pdf
                      </span>
                    </div>
                  </Link>
